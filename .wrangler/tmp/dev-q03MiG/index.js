...
async function sendEmail(apiKey, options) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[Email] Resend API error (${response.status}):`, responseText);
...
function tableRow(label, value, isLink = false) {
  const cellStyle = "padding: 8px; border: 1px solid #ddd;";
  const content = isLink ? `<a href="${value.includes("@") ? "mailto:" : "tel:"}${value}">${value}</a>` : value;
  return `
    <tr>
...
function optionalTableRow(label, value, isLink = false) {
  if (!value) return "";
  return tableRow(label, value, isLink);
}
__name(optionalTableRow, "optionalTableRow");
...
__name(notificationTable, "notificationTable");

// worker/validation.ts
var AU_PHONE_REGEX = /^(?:\+?61|0)[2-478](?:[ -]?\d){8}$/;
...
function validatePhone(phone, required = true) {
  const sanitized = sanitize(phone).replace(/\s/g, "");
  if (!sanitized) {
    if (required) {
...
function validateRequired(value, fieldName, maxLength) {
...
function validateOptional(value, fieldName, maxLength) {
...
function validateFile(file, required = false) {
  ...
  const isValidType = FILE_VALIDATION.allowedTypes.includes(file.type) || FILE_VALIDATION.allowedExtensions.includes(ext);
  if (!isValidType) {
...
function checkRateLimit(identifier, config = { maxRequests: 10, windowMs: 6e4 }) {
  const now = Date.now();
...
contact.post("/", async (c) => {
  ...
  try {
    ...
    const rateLimit = checkRateLimit(`contact:${clientIP}`, { maxRequests: 5, windowMs: 6e4 });
    ...
    const notificationRows = tableRow("Name", `${firstName} ${lastName}`) + optionalTableRow("Company", company) + tableRow("Email", email, true) + optionalTableRow("Phone", phone, true) + tableRow("Message", message);
...
quote.post("/", async (c) => {
  ...
  try {
    ...
    const rateLimit = checkRateLimit(`quote:${clientIP}`, { maxRequests: 5, windowMs: 6e4 });
    if (!rateLimit.allowed) {
    ...
    const notificationRows = tableRow("Name", `${firstName} ${lastName}`) + tableRow("Company", company) + tableRow("Email", email, true) + optionalTableRow("Phone", phone, true) + optionalTableRow("Sector", sector);
    const notificationHtml = emailWrapper(
      "New Quote Request",
      notificationTable(notificationRows)
    );
...
// worker/api/resume.ts
var resume = new Hono2();
...
resume.post("/", async (c) => {
  ...
  try {
    ...
    const rateLimit = checkRateLimit(`resume:${clientIP}`, { maxRequests: 5, windowMs: 6e4 });
    if (!rateLimit.allowed) {
    ...
    additionalInfo = additionalInfo ? sanitize(additionalInfo) : void 0;
    const firstNameValidation = validateRequired(firstName, "First name", MAX_LENGTHS.firstName);
    if (!firstNameValidation.valid) {
      return c.json({ error: firstNameValidation.error }, 400);
    }
    ...
    const notificationRows = tableRow("Name", `${firstName} ${lastName}`) + tableRow("Email", email, true) + tableRow("Phone", phone, true) + optionalTableRow("Additional Information", additionalInfo) + (fileAttachment ? tableRow("Resume", `${fileAttachment.filename} (attached)`) : "");
    const notificationHtml = emailWrapper(
    ...
    const notificationResult = await sendEmail(c.env.RESEND_API_KEY, {
      ...
      html: notificationHtml,
      replyTo: email,
      attachments: fileAttachment ? [fileAttachment] : void 0
    });
    if (!notificationResult.ok) {
      return c.json({ error: "Failed to send notification email" }, 500);
    }
...
application.post("/", async (c) => {
  ...
  try {
    ...
    const rateLimit = checkRateLimit(`application:${clientIP}`, { maxRequests: 10, windowMs: 6e4 });
    if (!rateLimit.allowed) {
    ...
    if (file && file.size > 0) {
      const arrayBuffer = await file.arrayBuffer();
      const base64Content = arrayBufferToBase642(arrayBuffer);
      fileAttachment = {
        filename: file.name,
        content: base64Content
      };
    ...
    const notificationRows = tableRow("Name", `${firstName} ${lastName}`) + tableRow("Email", email, true) + tableRow("Phone", phone, true) + tableRow("Position", `<a href="${jobUrl}" style="color: #141B2D;">${jobTitle}</a>`) + (fileAttachment ? tableRow("Resume", `${fileAttachment.filename} (attached)`) : tableRow("Resume", "Not provided"));
    const notificationHtml = emailWrapper(
      "New Job Application",
      notificationTable(notificationRows)
    );
...
app.route("/api/quote", quote_default);
app.route("/api/resume", resume_default);
app.route("/api/application", application_default);
app.all("*", async (c) => {
