/**
 * One-time migration script: converts static job data (Lexical-style rich text)
 * to Sanity documents (Portable Text).
 *
 * Usage: npx tsx scripts/migrate-jobs.ts
 *
 * Required env vars:
 *   SANITY_PROJECT_ID
...
// ---- Static job data (copied from src/data/jobs.ts before deletion) ----
...
interface StaticJob {
  ...
  sector: string
  salary?: string
  company_desc?: LexicalContent
  role_desc: LexicalContent
  role_requirements?: LexicalContent
  isActive: boolean
  createdAt: string
  updatedAt: string
}
...
/**
 * Convert Lexical-style rich text nodes to Portable Text blocks.
 */
function convertToPortableText(content: LexicalContent | undefined): PTBlock[] | undefined {
  if (!content?.root?.children) return undefined

  const blocks: PTBlock[] = []

  function processNodes(nodes: LexicalNode[], listItem?: string, level?: number) {
    for (const node of nodes) {
      if (node.type === 'paragraph') {
        const { spans, markDefs } = extractSpans(node.children || [])
        blocks.push({
          _type: 'block',
          _key: randomKey(),
          style: 'normal',
        ...
        for (const item of node.children || []) {
          if (item.type === 'listitem') {
            processListItem(item, listType, level || 1)
          }
        }
  ...
  function processListItem(node: LexicalNode, listItem: string, level: number) {
    // List items may contain paragraphs or direct text
    const textChildren: LexicalNode[] = []
    ...
    for (const child of node.children || []) {
      if (child.type === 'list') {
        ...
        textChildren.push(...(child.children || []))
      } else {
        textChildren.push(child)
      }
    }

    const { spans, markDefs } = extractSpans(textChildren)
    blocks.push({
      _type: 'block',
      _key: randomKey(),
      style: 'normal',
    ...
    for (const nestedList of nestedLists) {
      const nestedListType = nestedList.listType === 'number' ? 'number' : 'bullet'
      for (const item of nestedList.children || []) {
        if (item.type === 'listitem') {
          processListItem(item, nestedListType, level + 1)
        }
      }
    }
  }

  function extractSpans(nodes: LexicalNode[]): { spans: PTSpan[]; markDefs: PTMarkDef[] } {
    const spans: PTSpan[] = []
    ...
    for (const node of nodes) {
      if (node.type === 'text') {
        const marks: string[] = []
        if (node.format === 'bold') marks.push('strong')
        if (node.format === 'italic') marks.push('em')
  ...
  processNodes(content.root.children)
  return blocks.length > 0 ? blocks : undefined
}

// ---- The 7 static jobs ----

const jobs: StaticJob[] = [
  {
    id: '1',
    ...
    company_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'Our client is a well-established facade and window manufacturer with over 20 years of experience delivering high-quality products to the Australian construction industry. They pride themselves on innovation, quality craftsmanship, and exceptional customer service.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'With a strong reputation in both commercial and residential markets, they offer a supportive team environment and genuine career progression opportunities.' }] }
        ]
      }
    },
    role_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'As a Facade & Window Sales Representative, you will be responsible for developing and maintaining client relationships across the Sydney metropolitan area. This is a fantastic opportunity for a motivated sales professional to join a growing business.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'Key responsibilities include:' }] },
  ...
  },
  {
    ...
    company_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'A leading Australian window manufacturer with state-of-the-art production facilities is seeking an experienced Factory Manager to drive operational excellence.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'The company has built a strong reputation for quality aluminium and timber windows, supplying major residential and commercial projects across NSW.' }] }
        ]
      }
    },
    role_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'As Factory Manager, you will oversee all aspects of the window manufacturing operation, ensuring production targets are met while maintaining the highest quality standards.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'Key responsibilities include:' }] },
          { type: 'list', tag: 'ul', listType: 'bullet', children: [
  ...
  },
  {
    ...
    company_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'Our client is a highly regarded facade contractor specialising in curtain wall, cladding, and glazing systems for commercial and high-rise projects.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'With a portfolio of landmark buildings across Sydney, they offer an exciting opportunity to work on some of the most prestigious projects in the city.' }] }
        ]
      }
    },
    role_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'As a Facade Estimator, you will be responsible for preparing accurate and competitive estimates for commercial facade projects.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'Key responsibilities include:' }] },
          { type: 'list', tag: 'ul', listType: 'bullet', children: [
  ...
  },
  {
    ...
    company_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'Our client is a leading building consultancy specialising in facade diagnostics, defect assessment, and remedial engineering solutions.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'With a team of highly qualified engineers and consultants, they provide expert services to strata managers, developers, and building owners across NSW.' }] }
        ]
      }
    },
    role_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'As a Remedial Diagnostic Engineer & Building Consultant, you will conduct thorough building investigations and provide expert recommendations for remediation works.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'Key responsibilities include:' }] },
          { type: 'list', tag: 'ul', listType: 'bullet', children: [
  ...
  },
  {
    ...
    company_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'A market-leading facade and louvre contractor with an outstanding reputation for delivering complex commercial projects across Australia.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'The company is known for its innovative approach, strong safety culture, and commitment to employee development. Current projects include several landmark developments in the Sydney CBD.' }] }
        ]
      }
    },
    role_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'As a Senior Project Manager, you will take full ownership of major facade and louvre projects from award to completion, ensuring delivery on time, within budget, and to the highest quality standards.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'Key responsibilities include:' }] },
          { type: 'list', tag: 'ul', listType: 'bullet', children: [
  ...
  },
  {
    ...
    company_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'Our client is a rapidly growing facade installation company specialising in commercial glazing, curtain wall, and structural glass systems.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'Due to a strong pipeline of projects, they are expanding their team and seeking experienced glazing professionals at all levels.' }] }
        ]
      }
    },
    role_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'We are recruiting for multiple positions including Install Manager and Senior Glaziers to support the company\'s growing project portfolio.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'Install Manager responsibilities:' }] },
          { type: 'list', tag: 'ul', listType: 'bullet', children: [
          ...
          { type: 'paragraph', children: [{ type: 'text', text: 'Glazier responsibilities:' }] },
          { type: 'list', tag: 'ul', listType: 'bullet', children: [
    ...
    role_requirements: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'Install Manager requirements:' }] },
          { type: 'list', tag: 'ul', listType: 'bullet', children: [
          ...
          { type: 'paragraph', children: [{ type: 'text', text: 'Glazier requirements:' }] },
          { type: 'list', tag: 'ul', listType: 'bullet', children: [
  ...
  },
  {
    ...
    company_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'A highly respected facade contractor with a reputation for technical excellence and innovative design solutions.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'The company works on premium commercial and residential projects, and is known for pushing the boundaries of facade design and engineering.' }] }
        ]
      }
    },
    role_desc: {
      root: {
        type: 'root',
        children: [
          { type: 'paragraph', children: [{ type: 'text', text: 'As a Facade Drafter/Designer, you will produce high-quality shop drawings and design documentation for complex facade projects.' }] },
          { type: 'paragraph', children: [{ type: 'text', text: 'Key responsibilities include:' }] },
          { type: 'list', tag: 'ul', listType: 'bullet', children: [
...
async function main() {
  ...
  console.log(`Migrating ${jobs.length} jobs to Sanity (${projectId}/${dataset})...\n`)

  const mutations = jobs.map((job) => ({
    create: {
      _type: 'job' as const,
      title: job.title,
      ...
      sector: job.sector,
      salary: job.salary,
      company_desc: convertToPortableText(job.company_desc),
      role_desc: convertToPortableText(job.role_desc),
      role_requirements: convertToPortableText(job.role_requirements),
  ...
  if (!response.ok) {
    ...
    console.error(`Sanity API error (${response.status}): ${text}`)
    process.exit(1)
  }

  const result = await response.json() as { results: { id: string; operation: string }[] }

  console.log('Migration complete! Created documents:')
  for (let i = 0; i < result.results.length; i++) {
    const r = result.results[i]
    console.log(`  ${jobs[i].title} → ${r.id} (${r.operation})`)
  }
}
